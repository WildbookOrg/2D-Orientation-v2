...
class Perspective(object):
    ...
    def __init__(
        self, randint, distortion_scale=0.05, p=0.5, interpolation=Image.BICUBIC
    ):
        self.randint = randint
        self.p = p
    ...
    def get_params(self, width, height, distortion_scale):
        ...
        topleft = (
            np.random.uniform(0, int(distortion_scale * half_width)),
            np.random.uniform(0, int(distortion_scale * half_height)),
        )
        topright = (
            np.random.uniform(width - int(distortion_scale * half_width) - 1, width - 1),
            np.random.uniform(0, int(distortion_scale * half_height)),
        )
        botright = (
            np.random.uniform(width - int(distortion_scale * half_width) - 1, width - 1),
...
class Data:
    def __init__(self, args, type='train'):
        ...
        keypoints = np.genfromtxt(
            self.dataDir + 'keypoints.csv', delimiter=',', skip_header=True, dtype=str
        )
        ...
        train_indices, val_indices, test_indeces = np.split(
            permutation_order, [int(n * 0.6), int(n * 0.8)]
        )
    ...
    def __getitem__(self, index):
        ...
        if self.args.augment and self.type.startswith('train'):
            image_normalized, augmented_keypoints = self.augment_image_train(
                im, keypoints
            )
        else:
            image_normalized, original, augmented_keypoints = self.augment_image_test(
                im, keypoints
            )
            I = np.array(original)
    ...
    def augment_image_train(self, image, keypoints):
        ...
        persp_randint = 0.0  # np.random.uniform()
        ...
        keypoints[list(range(0, self.args.nClasses, 2))] = (
            keypoints[list(range(0, self.args.nClasses, 2))] * self.args.resize_to / w
        )
        keypoints[list(range(1, self.args.nClasses, 2))] = (
            keypoints[list(range(1, self.args.nClasses, 2))] * self.args.resize_to / h
        )
    ...
    def show_keypoints(self, image, keypoints, show_keypoints=True):
        ...
        if show_keypoints:
            plt.scatter(
                keypoints[list(range(0, self.nClasses, 2))],
                keypoints[list(range(1, self.nClasses, 2))],
                s=24,
                marker='.',
                c='r',
            )
    ...
    def review_good_images(self):
        ...
        for index in range(0, len(self.training_filenames)):
            try:
                ...
                for x, y in zip(
                    self.training_keypoints[index, :][::2],
                    self.training_keypoints[index, 1:][::2],
                ):
                ...
                if k == 98:  # b
                    ...
                    progress_file.write(
                        str(index) + ',' + self.training_filenames[index] + '\n'
                    )
                if k == 113:  # q
                    break
                if k == 99:  # c
    ...
    def review_bad_images(self):
        for index in range(len(self.bad_filenames)):
            try:
                ...
                for x, y in zip(
                    self.bad_keypoints[index, :][::2], self.bad_keypoints[index, 1:][::2]
                ):
                    cv2.circle(im, (int(float(x)), int(float(y))), 5, (0, 0, 255), -1)
                cv2.imshow('whale', im)
                k = cv2.waitKey(0)
