...
def network_preprocessing(model, args):
    ...
    # choice to resume training
        ...
        # no file found, exit
            ...
            print(
                'No state dict file found... (use --no-resume to begin training from scratch)'
            )
            exit(1)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--percent-error',
        action='store_true',
        help='While calculating loss and error, use value from [0,1] instead of pixel distance',
    )
    parser.add_argument(
        '--test',
        action='store_true',
    ...
    parser.add_argument(
        '--debug', action='store_true', help='show debugging print statements'
    )
    parser.add_argument(
        '--augment',
        action='store_true',
        help='augment images used in training (perspective transform)',
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help='specify the number of classes, for fluke tip regression this number must be even in the range of [2,20]',
    )
    parser.add_argument(
        '--nEpochs', type=int, default=50, help='specify a number of epochs'
    )
    parser.add_argument(
        '--resize-to', type=int, default=128, help='specify the image size used'
    )
    parser.add_argument('--lr', type=float, default=1e-4, help='specify a learning rate')
    parser.add_argument(
        '--type',
        default='regression_fluke_tips',
        help='specify a network type to be used with directory saving',
    )
    parser.add_argument(
        '--vinvine',
        action='store_true',
        help='some fun example that isnt expected to work',
    )
    parser.add_argument(
        '--train-bad-images',
        action='store_true',
        help='dont remove labeled poor images before training',
    )
    parser.add_argument(
        '--save-all-figures',
        action='store_true',
    ...
    dataloaders = {
        ...
        'test': DataLoader(
            data['test'], batch_size=args.batch_size, shuffle=False, drop_last=True
        ),
    }
    ...
    loss_history = {'train': [[], []], 'val': [[], []], 'save': [[], []]}

    # then train
    # best value loss calculated from the previous training session weights
    best_val_loss = train2(
        args, 0, model, dataloaders['val'], datafiles['val'], loss_func, 'val'
    )
    ...
    for epoch in range(args.nEpochs):
        ...
        for phase in ['train', 'val']:
            ...
            val_loss = train2(
                args, epoch, model, dataloaders[phase], datafiles[phase], loss_func, phase
            )
