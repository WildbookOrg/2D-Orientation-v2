...
def print_statistics(epoch, nProcessed, nTrain, batch_idx, loss, err, datafile, phase):
	...
	print(phase+': {:.2f} [{:.2f}/{:.2f} ({:.2f}%)]\tLoss: {:.2f}\tError: {:.2f}'.format(
		partialEpoch, nProcessed, nTrain, 100. * nProcessed / nTrain,(loss.item()), err))
...
def train(args, epoch, model, dataloader, datafile, loss_func, phase):
	...
	val_loss = torch.Tensor([0.,0.,0.,0.]).cuda()
	nProcessed = 0
	...
	with torch.set_grad_enabled(phase == 'train'):
		for batch_idx, (data, target) in enumerate(dataloader):
			...
			# val_loss += loss.data
			print_statistics(epoch, nProcessed, nTrain, batch_idx, torch.mean(loss.data), err, datafile, phase)
	return (val_loss)	
...
def test(args, model, dataloader, datafile, loss_func, phase):
	...
	with torch.set_grad_enabled(False):
		for batch_idx, (data, target) in enumerate(dataloader):
			...
			print_statistics(0, nProcessed, nTrain, batch_idx, torch.mean(loss.data), err, datafile, phase)
	...
	# plot histogram
	plt.hist(x1, label='Left Tip X', alpha=0.4, bins = np.linspace(np.min(x1),np.max(x1),num=50))
