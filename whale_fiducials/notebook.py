...
def show_keypoints(image, keypoints):
    ...
    if len(keypoints):
        plt.scatter(
            keypoints[[0, 2, 4, 6]], keypoints[[1, 3, 5, 7]], s=24, marker='.', c='r'
        )
...
class MLP(nn.Module):
    def __init__(self, input_size, output_size, hidden_layers, drop_p=0.5):
        ...
        # hidden layers
        layer_sizes = [(input_size, hidden_layers[0])] + list(
...
model = MLP(
    input_size=IMG_SIZE * IMG_SIZE, output_size=20, hidden_layers=[128, 64], drop_p=0.1
)
...
def view_pred_df(
    filenames, predictions, ncols=5, figsize=(15, 10), image_ids=range(1, 6)
):
    """
