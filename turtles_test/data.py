...
class Data_turtles:
    def __init__(self, dataDir=None, dataType='train2020', experiment_type='', args=None):
        if dataDir == None:
            self.dataDir = '../datasets/{}/orientation.{}.coco'.format(
                args.animal, args.animal
            )
    ...
    def __getitem__(self, index):
        ...
        h, w = image.shape[:2]
        cx, cy = w / 2, h / 2
        frame = np.array([[0, 0], [w, 0], [w, h], [0, h]])
    ...
    def show_MER(self, enclosing):
        ...
        p = PatchCollection(
            polygons, facecolor='none', edgecolors=(1, 0, 0), linewidths=2
        )
    ...
    def test_image(self, image, poly, theta):
        ...
        I = transforms.functional.affine(I, angle, (0, 0), 1, 0)
        I = transforms.ToTensor()(I)
        image_normalized = transforms.Normalize((1, 1, 1), (0.5, 0.5, 0.5))(I)
        return True
    ...
    def preprocess_images(self):
        ...
        if os.path.isfile(self.dataDir + '/loaded_data_{}.p'.format(self.dataType)):
            ...
            (self.data, self.means, self.stds) = pickle.load(
                open(self.dataDir + '/loaded_data_{}.p'.format(self.dataType), 'rb')
            )
        ...
        for i, ID in enumerate(self.imgIds):
            ...
            img = self.coco.loadImgs(ID)[0]
            I = io.imread(
                '%s/images/%s/%s' % (self.dataDir, self.dataType, img['file_name']), False
            )
            # plt.axis('off')
            ...
            try:
                self.test_image(
                    I, poly, animal_head['theta']
                )  # ,animal_head['viewpoint'])
            except:
                print('\nskipped', ID)
                continue
        ...
        pickle.dump(
            (self.data, means, stds),
            open(self.dataDir + '/loaded_data_{}.p'.format(self.dataType), 'wb'),
        )
