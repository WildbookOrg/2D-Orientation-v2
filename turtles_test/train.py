...
def example_function(model, args):
    ...
    images = [
        transforms.functional.affine(image, -angle, (0, 0), 1, 0)
        for image, angle in zip(images, pred)
    ]
    ...
    plt.title(
        args.type
        + '\nPred: {}\nTrue: {}'.format(
            angles.detach().cpu().numpy(), pred.detach().cpu().int().numpy()
        )
    )
    plt.show()
    ...
    for images_normalized, images, angles in testLoader:
        ...
        for i in range(args.batchSz):
            if angles[i] - pred[i] > 7:
                ...
                plt.axis('off')
                plt.title(
                    'Regression Trig Loss'
                    + '\nTrue: '
                    + str(int(angles[i]))
                    + '       \nPred: {:.3f}'.format(float(pred[i]))
                )
                plt.savefig(
                    './results/turtle/reg_trig_figures/bad_example{}.png'.format(
                        str(i_major + i)
                    )
                )
...
def get_data_loaders(args):
    ...
    val_dataset = Data_turtles(
        dataType='val2020', experiment_type='validation', args=args
    )
    test_dataset = Data_turtles(dataType='test2020', experiment_type='test', args=args)

    dataloaders = {
        'train': DataLoader(
            train_dataset, batch_size=args.batchSz, shuffle=True, drop_last=True
        ),
        'val': DataLoader(
            val_dataset, batch_size=args.batchSz, shuffle=True, drop_last=True
        ),
    }
    return dataloaders
...
def get_optimizer(model, args):
    if args.opt == 'sgd':
        return optim.SGD(model.parameters(), lr=1e-1, momentum=0.9, weight_decay=1e-4)
    elif args.opt == 'adam':
...
def plot_loss_history(args):
    ...
    for i in range(len(train_partial_epoch_split)):
        plt.plot(
            train_partial_epoch_split[i],
            train_partial_loss_split[i],
            label='train partial',
            color='b',
        )
    for i in range(len(val_partial_epoch_split)):
        plt.plot(
            val_partial_epoch_split[i],
            val_partial_loss_split[i],
            label='val partial',
            color='r',
        )
...
def main():
    ...
    parser.add_argument(
        '--nEpochs', type=int, default=25, help='specify a number of epochs'
    )
    parser.add_argument(
        '--opt', type=str, default='adam', choices=('sgd', 'adam', 'rmsprop')
    )
    parser.add_argument(
        '--no-resume',
        action='store_true',
        help='delete current trained weights and train from scratch',
    )
    parser.add_argument(
        '--example',
        action='store_true',
        help='show a visualization of a sample of the test set',
    )
    parser.add_argument(
        '--type',
        default='regression',
        help='specify a network type to be used with directory saving',
    )
    parser.add_argument(
        '--test',
        action='store_true',
        help='test the current weights and plot some statistics',
    )
    parser.add_argument(
        '--nClasses',
        type=int,
        default=1,
        help='specify the number of classes (must be 1 for regression)',
    )
    parser.add_argument(
        '--pretrain',
        action='store_true',
        help='used with --no-resume, load wiped network with densenet pretrained values',
    )
    parser.add_argument(
        '--separate-trig',
        action='store_true',
        help='use the trig components (sin,cos) to estimate rotation, rather than theta',
    )
    parser.add_argument(
        '--degree-loss',
        action='store_true',
        help='estimate orientation using sin cos components',
    )
    parser.add_argument(
        '--device',
        type=int,
        default=0,
        choices=tuple(range(torch.cuda.device_count())),
        help='specify the cuda device to run on',
    )
    parser.add_argument(
        '--save-all-figs',
        action='store_true',
        help='apply the example function to each test image and save in a directory',
    )  # TODO
    parser.add_argument(
        '--animal',
        type=str,
    ...
    parser.add_argument(
        '--show',
        action='store_true',
        help='shows each image during dataloader stage to see annotation',
    )
    parser.add_argument('--plot-loss-history', action='store_true')

    parser.add_argument(
        '--lr', type=float, default=1e-4, help='specify the initial learning rate'
    )
    parser.add_argument(
        '--angle-range',
        type=int,
        default=360,
        help='angle range to train with; within [10,360]',
    )
    parser.add_argument(
        '--hierarchy',
        action='store_true',
        help='use the hierarchy model and combine classification and regression',
    )
    ...
    args.save_path = 'work/densenet.%s.%s.%s%s/' % (
        args.animal,
        args.type,
        args.nClasses,
    ...
    if args.type.startswith('classification') or args.hierarchy:
        model.classifier = nn.Sequential(
            nn.Linear(2208, args.nClasses), nn.LogSoftmax(dim=0)
        )
    ...
    if args.hierarchy:
        ...
        best_val_loss, best_val_loss_reg = train_hierarchy(
            args,
            0,
            [model, model_reg],
            dataloaders['val'],
        ...
        for epoch in range(1, args.nEpochs + 1):
            ...
            for phase in ['train', 'val']:
                val_loss, val_loss_reg = train_hierarchy(
                    args,
                    epoch,
                    [model, model_reg],
                    dataloaders[phase],
...
def train(args, epoch, net, dataloader, optimizer, datafile, loss_func, phase):
    ...
    for batch_idx, (data, target) in enumerate(dataloader):
        ...
        if args.type.startswith('regression'):
            ...
            if args.separate_trig:
                err = torch.mean(
                    abs(output.float() - angle_to_separate_trig(target.float(), args))
                )
            else:
...
def train_hierarchy(args, epoch, net, dataloader, optimizer, datafile, loss_func, phase):
    ...
    for batch_idx, (data, target) in enumerate(dataloader):
        ...
        target = torch.Tensor(
            [int(angle / int(360 / args.nClasses)) for angle in target]
        ).long()
        ...
        data = [
            transforms.functional.affine(image, -angle, (0, 0), 1, 0)
            for image, angle in zip(data, pred)
        ]
...
def test(args, epoch, net):
    ...
    dataloader = DataLoader(
        test_dataset, batch_size=args.batchSz, shuffle=False, drop_last=True
    )
