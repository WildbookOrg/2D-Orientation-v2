...
def example(args):
    ...
    if not os.path.exists(args.save):
        ...
        exit(1)
    net = DenseNet(
        growthRate=12, depth=100, reduction=0.5, bottleneck=True, nClasses=args.nClasses
    )
    print('Loading Saved Parameters...')
    if args.cuda:
        net.load_state_dict(
            torch.load(os.path.join(args.save, 'latest-%s.pth' % (args.type)))
        )
    else:
        net.load_state_dict(
            torch.load(
                os.path.join(args.save, 'latest-%s.pth' % (args.type)),
                map_location=lambda storage, location: 'cpu',
            )
        )

    test_dataset = Data(path='./data/mnist/mnist_test.csv', train=False, example=True)
    if args.batchSz > 8:
        args.batchSz = 8
