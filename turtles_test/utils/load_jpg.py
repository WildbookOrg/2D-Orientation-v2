...
class Data:
    def __init__(
        self, filenames, path='./data/seaturtle.coco/images/train2019/', train=True
    ):
        self.data = filenames
        self.train = train
...
def get_filenames(json_file):
    ...
    filename = next(
        (image['file_name'] for image in annotations['images'] if image['id'] == 28), None
    )
...
def main():
    ...
    dataset = Data(filenames=filenames, train=True)
